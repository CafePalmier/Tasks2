...
create table if not exists public.cafe_tasks (
  ...
  season text not null default 'both' check (season in ('winter', 'summer', 'both')),
...
alter table public.cafe_tasks add column if not exists season text not null default 'both';
